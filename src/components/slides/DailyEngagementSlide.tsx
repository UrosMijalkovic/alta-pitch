        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-base font-semibold text-white">Weekly Streak</h3>
                <p className="text-xs text-[#8A94A6]">Day {currentDay + 1} of 7</p>
              </div>
              <div className="flex items-center gap-2 bg-[#52AE30]/20 rounded-full px-2 py-0.5">
                ...
                <span className="text-[#52AE30] font-semibold text-xs">{currentDay + 1} day streak!</span>
              </div>
            </div>
                ...
                >
                  {reward.special && (
                    <div className="absolute -top-1 -right-1">
                      <svg className="w-3 h-3 text-[#C5A572]" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    </div>
                  )}
                  <span className="text-[10px] text-[#8A94A6] mb-0.5">{weekDays[index]}</span>
                  {reward.claimed ? (
                    <div className="w-5 h-5 rounded-full bg-[#52AE30] flex items-center justify-center">
                      <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                  ) : (
                    <span className={`text-xs font-bold ${index === currentDay ? 'text-white' : 'text-[#8A94A6]'}`}>
                      +{reward.xp}
                    </span>
                  )}
                </motion.div>
            ...
            {/* Streak message */}
            <motion.div
              className="mt-3 p-2 bg-gradient-to-r from-[#52AE30]/10 to-transparent rounded-lg border-l-4 border-[#52AE30]"
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-base font-semibold text-white">Monthly Challenge</h3>
                <p className="text-xs text-[#8A94A6]">18 of 30 days completed</p>
              </div>
              <div className="text-right">
                <p className="text-xl font-bold text-[#C5A572]">+500 XP</p>
                <p className="text-[10px] text-[#8A94A6]">completion bonus</p>
              </div>
            </div>
            ...
            <div className="mt-3">
              ...
              <div className="h-1.5 bg-[#1A222C] rounded-full overflow-hidden">
                <motion.div
            ...
            <div className="mt-3 flex justify-between">
              {[7, 14, 21, 30].map((day, index) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full mx-auto mb-0.5 flex items-center justify-center ${
                      day <= 18
                        ? 'bg-[#52AE30] text-white'
                  ...
                  <p className="text-[10px] text-[#8A94A6]">Day {day}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
        ...
        >
          {[
            ...
            <div key={stat.label} className="glass rounded-lg p-2 text-center">
              <p className="text-xl font-bold text-[#52AE30]">{stat.value}</p>
              <p className="text-[10px] text-[#8A94A6] mt-0.5">{stat.label}</p>
            </div>
          ))}
        </motion.div>
      </div>
